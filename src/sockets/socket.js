  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send-message", async (messageData) => {
      try {
        ...
        const message = await Message.create({ chat: chatId, sender: senderId, content, mediaURL, messageType });
        await Chat.findByIdAndUpdate(chatId, { lastMessage: message._id, updatedAt: new Date() });

        const populatedMessage = await Message.findById(message._id).populate("sender", "fullname phoneNumber avatarURL");
